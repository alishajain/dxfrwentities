...
void testWriting() {
	...
	while(!f.eof())
	{
		...
		else if(sel_str.compare("line")==0)
		{
			...
			dxf->writeLine(
        		*dw,
        		DL_LineData(x1, y1, 0.0,   // start point
                    			x2, y2, 0.0),   // end point
        		DL_Attributes("mainlayer", 256, -1, "BYLAYER"));
		}
    		...
    		{
			float x, y, rad, s_angle, e_angle;
			cout<<"********* ARC ***********"<<endl;
			f>>x>>y>>rad>>s_angle>>e_angle;
			dxf->writeArc(
		        *dw,
		        DL_ArcData(x,y,0.0,	// center point
                    			rad,   // radius
                    			s_angle,   // start angle
                    			e_angle),    // end angle
        		DL_Attributes("mainlayer", 256, -1, "BYLAYER"));
		...
		else if( sel_str.compare("circle") == 0)
   		{
			...
			dxf->writeCircle(
        		*dw,
        		DL_CircleData(x,y,0.0,		// center point
                    			rad),		// radius
		...
		else if( sel_str.compare("rectangle") == 0)
		{
			...
			dxf->writeLine(
        		*dw,
			DL_LineData(x2,y2, 0.0,   // start point
                    			x3, y3, 0.0),   // end point
        		DL_Attributes("mainlayer", 256, -1, "BYLAYER"));

			dxf->writeLine(
        		*dw,
			DL_LineData(x3, y3, 0.0,   // start point
                    			x4, y4, 0.0),   // end point
        		DL_Attributes("mainlayer", 256, -1, "BYLAYER"));

			dxf->writeLine(
        		*dw,
			DL_LineData(x4, y4, 0.0,   // start point
                    			x1, y1, 0.0),   // end point
        		DL_Attributes("mainlayer", 256, -1, "BYLAYER"));
		}
	}
